	...
	<div class="col-lg-12">
		<div class="row">
			...
			<form action="" id="manage-menu">
                            <div class="card" style="width: 302px; right: 100px; top: 20px">
					...
					<div class="card-body">
							...
							<div class="form-group">
								...
								<select name="category_id" id="" class="custom-select browser-default">
									<?php
									$cat = $conn->query("SELECT * FROM category_list where admin_id = $id order by name asc ");
			...
			<!-- Table Panel -->
			<div class="col-md-8">
                            <div class="card" style="width: 652px; right: 45px; top: 20px">
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								$i = 1;
                                                                $id=$_SESSION["login_admin_id"];
								$cats = $conn->query("SELECT * FROM product_list where admin_id = $id order by pro_id asc");
								...
								<tr>
									...
									<td class="text-center">
                                                                            <button class="btn btn-sm btn-primary edit_menu" type="button" data-id="<?php echo $row['pro_id'] ?>" data-name="<?php echo $row['name'] ?>"  data-description="<?php echo $row['description'] ?>" data-price="<?php echo $row['price'] ?>" data-cat_id='<?php echo $row['category_id'] ?>' data-img_path="<?php echo $row['img_path'] ?>">Edit</button>
										<button class="btn btn-sm btn-danger delete_menu" type="button" data-id="<?php echo $row['pro_id'] ?>">Delete</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('#manage-menu').submit(function(e){
		...
		start_load()
		$.ajax({
			url:'ajax.php?action=save_menu&admin_id=<?php echo $_SESSION["login_admin_id"] ?>',
	...
	$('.edit_menu').click(function(){
		...
		cat.find("[name='name']").val($(this).attr('data-name'))
                cat.find("[name='category_id']").val($(this).attr('data-cat_id'))
